...
export default function Membership() {
  ...
  // persist selected membership plan — local UI state before Zustand commit on checkout
  ...
  const goToCheckout = (planType) => {
    const cycle = cycleByPlan[planType];
    if (!cycle) {
    ...
    if (planType === 'student') {
      ...
      const ok = verifyStudentEmail(studentEmail.trim());
      if (!ok) {
        const err =
  ...
  const PlanCard = ({ planKey, badge }) => {
    const plan = plans[planKey];
    const cycleKey = cycleByPlan[planKey];
  ...
  return (
    ...
    <div className="max-w-5xl mx-auto pb-16 pt-2 md:pt-4 space-y-10">
      ...
      <div className="grid md:grid-cols-2 gap-6 md:gap-8">
        <PlanCard planKey="student" badge={null} />
        <PlanCard planKey="individual" badge="Popular" />
      </div>
